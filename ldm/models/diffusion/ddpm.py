...
def visualize_fea(save_path, fea_img):
    fig = plt.figure(
        figsize=(fea_img.shape[1] / 10, fea_img.shape[0] / 10)
    )  # Your image (W)idth and (H)eight in inches
    plt.subplots_adjust(left=0, right=1.0, top=1.0, bottom=0)
    im = plt.imshow(
        fea_img, vmin=0.0, vmax=1.0, cmap="jet", aspect="auto"
    )  # Show the image
...
class DDPM(pl.LightningModule):
    ...
    def q_sample_respace(
        self, x_start, t, sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod, noise=None
    ):
        noise = default(noise, lambda: torch.randn_like(x_start))
        return (
            extract_into_tensor(sqrt_alphas_cumprod.to(noise.device), t, x_start.shape)
            * x_start
            + extract_into_tensor(
                sqrt_one_minus_alphas_cumprod.to(noise.device), t, x_start.shape
            )
            * noise
        )
...
class LatentDiffusionSRTextWT(DDPM):
    ...
    def get_input(
        ...
        out = F.interpolate(
            out,
            size=(
                int(ori_h / self.configs.sf * scale),
        ...
        if (
            random.random() < self.configs.degradation["no_degradation_prob"]
            or torch.isnan(self.lq).any()
        ):
            self.lq = self.gt
    ...
    def p_mean_variance(
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.0:
            ...
            e_t_uncond, e_t = self.apply_model(
                x_in, t_in_, c_in, struct_cond, return_ids=False
            ).chunk(2)
            ...
            if t[0] >= reference_range[0] and t[0] <= reference_range[1]:
                ...
                for _ in range(reference_step):
                    with torch.enable_grad():
                        ...
                        tau0 = tau0 * mask
                        delta_y = (
                            torch.square(reference_sr - xstart_current).sum()
                            / reference_sr.shape[0]
                        )
                        gradient = (
                            torch.autograd.grad(delta_y, xstart_current)[0] * tau0
                        )
                        assert not torch.isnan(gradient).any()
                        new_xstart = xstart_current.float().detach() - gradient.float()
    ...
    def p_mean_variance_canvas(
            ...
            for col in range(grid_cols):
                ...
                if len(input_list) == batch_size or col == grid_cols - 1:
                    ...
                    if (
                        ...
                        struct_cond_input = self.structcond_stage_model(
                            cond_list, t_in[: input_list.size(0)]
                        )
                        model_out = self.apply_model(
                            input_list,
                            t_in[: input_list.size(0)],
                            c[: input_list.size(0)],
                            struct_cond_input,
                        ...
                        model_out = e_t_uncond + unconditional_guidance_scale * (
                            e_t - e_t_uncond
                        )
                        return_codebook_ids = False
        ...
        if self.parameterization == "eps":
            x_recon = self.predict_start_from_noise(
                x, t=t[: model_out.size(0)], noise=model_out
            )
        elif self.parameterization == "x0":
            x_recon = model_out
        elif self.parameterization == "v":
            x_recon = self.predict_start_from_z_and_v(
                x, model_out, t[: model_out.size(0)]
            )
        else:
            raise NotImplementedError()
        ...
        model_mean, posterior_variance, posterior_log_variance = self.q_posterior(
            x_start=x_recon, x_t=x, t=t[: x_recon.size(0)]
        )
        if return_codebook_ids:
            return model_mean, posterior_variance, posterior_log_variance, logits
        elif return_x0:
    ...
    def p_sample_canvas(
        ...
        b, *_, device = *x.shape, x.device
        outputs = self.p_mean_variance_canvas(
            x=x,
            c=c,
            struct_cond=struct_cond,
            t=t,
        ...
        if noise_dropout > 0.0:
            noise = torch.nn.functional.dropout(noise, p=noise_dropout)
        # no noise when t == 0
        nonzero_mask = (1 - (t[:b] == 0).float()).reshape(
            b, *((1,) * (len(x.shape) - 1))
        )
        ...
        if len(batch_list) > 0:
            ...
            for batch_i in range(num_batch):
                ...
                for num_i in range(len(batch_list)):
                    batch64_list.append(
                        cal_pca_components(batch_list[num_i]["64"][batch_i], 3)
                    )
                    batch32_list.append(
                        cal_pca_components(batch_list[num_i]["32"][batch_i], 3)
                    )
                ...
                for index in range(total_num):
                    os.makedirs(
                        os.path.join(interfea_path, "fea_" + str(batch_i) + "_64"),
                        exist_ok=True,
                    )
                    cur_path = os.path.join(
                        interfea_path,
                        "fea_" + str(batch_i) + "_64",
                        "step_" + str(total_num - index) + ".png",
                    )
                    visualize_fea(cur_path, batch64_list[index])
                    os.makedirs(
                        os.path.join(interfea_path, "fea_" + str(batch_i) + "_32"),
                        exist_ok=True,
                    )
                    cur_path = os.path.join(
                        interfea_path,
                        "fea_" + str(batch_i) + "_32",
                        "step_" + str(total_num - index) + ".png",
                    )
    ...
    def p_sample_loop_canvas(
        ...
        for i in iterator:
            if time_replace is None or time_replace == 1000:
                ...
                ts = torch.full((b,), i, device=device, dtype=torch.long)
                t_replace = repeat(
                    torch.tensor([self.ori_timesteps[i]]), "1 -> b", b=batch_size
                )
                t_replace = t_replace.long().to(device)
            if self.shorten_cond_schedule:
                assert self.model.conditioning_key != "hybrid"
        ...
        if cond is not None:
            if isinstance(cond, dict):
                cond = {
                    key: cond[key]
                    if not isinstance(cond[key], list)
                    else list(map(lambda x: x, cond[key]))
                    for key in cond
                }
            else:
                cond = [c for c in cond] if isinstance(cond, list) else cond
        return self.p_sample_loop_canvas(
            cond,
            struct_cond,
            shape,
            return_intermediates=return_intermediates,
    ...
    def log_images(
        ...
        **kwargs,
        ...
        log = dict()
        z, c_lq, z_gt, x, gt, yrec, xc = self.get_input(
            batch,
            self.first_stage_key,
            return_first_stage_outputs=True,
            force_c_encode=True,
    ...
    def forward(
        ...
        if self.conditioning_key is None:
            ...
            assert struct_cond is not None, (
                "struct_cond must be provided when conditioning_key is 'struct_only'"
            )
