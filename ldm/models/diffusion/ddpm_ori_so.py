...
class LatentDiffusionOriSO(LatentDiffusion):
    ...
    def forward(self, x, c, gt, *args, **kwargs):
        ...
        struct_c = self.cond_stage_model(x, t_ori)
        # struct_c = self.get_learned_conditioning(x,t_ori)  # [B, C, 64, 64]

        # 执行扩散损失计算
        # return self.p_losses(x, c, t, gt=gt, *args, **kwargs)
        return self.p_losses(gt, c, struct_c, t, t_ori, x, *args, **kwargs)
        ...
        for i in iterator:
            ...
            if t_replace is not None:
                ...
                if start_T is not None:
                    if i > start_T:
                        continue
                struct_cond_input = self.cond_stage_model(struct_cond, ts)

            # print(type(struct_cond_input))
            # print(struct_cond_input.keys())
