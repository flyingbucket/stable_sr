...
from ldm.models.autoencoder import AutoencoderKLResi
...
class AutoencoderKLPlus(pl.LightningModule):
    ...
    def _init_dgconv_modules(self, ddconfig):
        ...
        if "encoder_output" in self.dgconv_positions:
            ...
            self.dgconv_modules["encoder_output"] = self._create_dgconv(
        ...
        if "decoder_input" in self.dgconv_positions:
            ...
            self.dgconv_modules["decoder_input"] = self._create_dgconv(
                decoder_in_channels
            )
    ...
    def encode(self, x, return_encfea=False):
        ...
        # 在encoder输出处应用DGConv
        if (
            "encoder_output" in self.dgconv_positions
            and "encoder_output" in self.dgconv_modules
        ):
    ...
    def decode(self, z):
        ...
        # 在decoder输入处应用DGConv
        if (
            "decoder_input" in self.dgconv_positions
            and "decoder_input" in self.dgconv_modules
        ):
    ...
    def configure_optimizers(self):
        ...
        # 包含DGConv模块的参数
        dgconv_params = (
            list(self.dgconv_modules.parameters())
            if hasattr(self, "dgconv_modules")
            else []
        )
...
class AutoencoderKLResiPlus(AutoencoderKLResi):
    def __init__(
        ...
        if (
            "decoder_input" in self.dgconv_positions
            or "decoder_middle" in self.dgconv_positions
        ):
    ...
    def encode(self, x):
        ...
        if "encoder_middle" in self.dgconv_positions and hasattr(
            self, "enhanced_encoder"
        ):
